...
import { Dimensions, Pressable, ScrollView, Text, TouchableHighlight, View } from 'react-native';
...
import { faChevronCircleRight, faMapPin, faSearch } from '@fortawesome/free-solid-svg-icons';
...
export default function EditLocation() {
  ...
  const fetchSuggestAddress = useDebouncedCallback(async (value: any) => {
    ...
    const { items }: any = await apiService.get(
      ...
      }&limit=3&lang=en&q=${`${value} ${subValue}`}&apiKey=${process.env.EXPO_PUBLIC_HERE_API_KEY
      }`
    );
  ...
  const selectedState = () => (
    <View
      ...
      {Object.entries(addressForm).map(([key, value]: any, idx: number) => (
        <TouchableHighlight
          key={idx}
          onPress={() => handleChangeAddress(key)}
        >
          <View style={{
            display: 'flex',
            flexDirection: 'row',
    ...
    <View
      ...
      <View
        ...
        <Pressable
          ...
          onPress={() => (currentEdit ? handleReturn() : router.replace('/drop/upload-image'))}>
          <Svg
            width='17'
            height='16'
            viewBox='0 0 17 16'
            ...
            <>
              <View
                ...
                >
                  ...
                  <FontAwesomeIcon icon={faChevronCircleRight} size={16} color='#ffffff70' />
                </Pressable>
                <Pressable
                  style={{
                    borderRadius: 12,
            ...
            <>
              {currentEdit !== 'confirm' && (
                ...
                >
                  <CustomiseInputWIco
                    ...
                    leftIco={
                      currentEdit !== 'address' && (
                        <FontAwesomeIcon icon={faSearch} size={16} color='#ffffff70' />
                      )
                    }
                  />
                  <>
              ...
              {currentEdit === 'confirm' && (
                ...
                >
                  ...
                  >
                    <Marker
                      coordinate={{ longitude: addressLocation.lng, latitude: addressLocation.lat }}
                    >
