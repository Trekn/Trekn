...
export default function DetailCard({ data, status, last }: { data: any; status?: any; last?: boolean }) {
  ...
  return (
    <View style={{ paddingBottom: isHome() ? 24 : !last ? 48 : 0 }}>

      {isHome() && lastChecked && (
        <View style={{ marginBottom: 3 }}>
          <Text style={{ fontSize: 13, color: 'black', opacity: 0.7 }}>
            <FontAwesomeIcon size={13} icon={faMapPin} style={{ marginRight: 4 }} /> {lastChecked.name} checkin{' '}
            {moment(lastChecked.minted[0].created_at).startOf('hour').fromNow()}
          </Text>
        </View>
      )}

      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }} key={data.id}>
        <Pressable onPress={() => data?.user.id !== user.id && router.replace(`/account/${data.user.id}`)}>
          {data.user.profile_image || data.user.profileImage ?
            <Image
              source={{ uri: data.user.profileImage || data.user.profile_image }}
              style={{ width: 40, height: 40, borderRadius: 20, marginRight: 8 }}
            />
            :
            <Svg viewBox="0 0 36 36" fill="none" role="img" width="40" height="40" style={{ width: 40, height: 40, borderRadius: 20, marginRight: 8 }} >
              <Mask id=":r6:" maskUnits="userSpaceOnUse" x="0" y="0" width="36" height="36">
                <Rect width="36" height="36" rx="72" fill="#FFFFFF"></Rect>
              </Mask>
              <G mask="url(#:r6:)">
                <Rect width="36" height="36" fill="#ff7d10" />
                <Rect x="0" y="0" width="36" height="36" transform="translate(4 4) rotate(340 18 18) scale(1.1)" fill="#0a0310" rx="36" />
                <G transform="translate(-4 -1) rotate(0 18 18)">
                  <Path d="M15 20c2 1 4 1 6 0" stroke="#FFFFFF" fill="none" stroke-linecap="round"></Path>
        ...
        <View style={{ flexDirection: 'column' }}>
          <Text style={{ fontWeight: '500', marginBottom: 8, fontSize: 15, lineHeight: 16 }}>{data.user.name}</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <Text style={{ fontWeight: '500', color: 'black', opacity: 0.5, marginLeft: 4, fontSize: 13, lineHeight: 16 }}>
      ...
      >
        <View style={{ position: 'relative', height: 480, width: '100%' }}>
          {data?.drop?.imageArray || data?.imageArray ?
            <CustomCarousel data={data} setCurrentImg={setCurrentImg} />
            :
            <Image
          ...
          <Pressable
            ...
            <BlurView
              ...
              tint='dark'>
              <Image
                source={data?.drop?.imageArray || data?.imageArray ? (data.imageArray ? data.imageArray : data?.drop?.imageArray)[currentImg] : data?.image || data?.drop?.image}
                style={{
                  height: '100%',
                  width: '100%',
                }}
            ...
            <Text style={{ fontWeight: '600', fontSize: 16, lineHeight: 24, color: 'white', marginBottom: 8 }}>
              {data?.name || data?.drop?.name}
            </Text>
            <View style={{ flexDirection: 'row', columnGap: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', columnGap: 4 }}>
                <FontAwesomeIcon color='#FFB800' size={16} icon={faThumbsUp} />
                <Text style={{ color: 'white', opacity: 0.7, fontSize: 14, fontWeight: '500', lineHeight: 20 }}>{getScore(data, false)}</Text>
              </View>
              <Text style={{ color: 'white', opacity: 0.7, fontSize: 14, lineHeight: 20, fontWeight: '500' }}>
                <Text style={{ color: 'white', lineHeight: 20, fontWeight: '500' }}>●</Text>{' '}
                {convertDistance(
                  calculateDistance(
            ...
            {userChecked.length > 0 &&
              <View style={{ marginTop: 16, flexDirection: 'row', alignItems: 'center', columnGap: 8 }}>
                ...
                <View style={{ backgroundColor: 'white', paddingHorizontal: 8, paddingVertical: 6, borderRadius: 15 }}>
                  <Text style={{ fontSize: 13, fontWeight: '500', color: 'black', lineHeight: 13 }}>{userChecked.length} check-ins</Text>
                </View>
              </View>
            }
          </Pressable>
      ...
      {!last && <View style={{ borderBottomWidth: 1, borderColor: '#EAEAEA', marginTop: 24 }} />}
